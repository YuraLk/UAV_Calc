...
import (
	...
	battery "github.com/YuraLk/teca_server/internal/dtos/battery"
)

type FileService struct{}

func (FileService) ParseTableFromFile(file *multipart.FileHeader) ([]battery.BatteryDto, error) {

	// Открываем файл
	src, err := file.Open()

	if err != nil {
		return []battery.BatteryDto{}, err
	}
	defer src.Close()
	...
	for {
		...
		CVC = append(CVC, battery.BatteryDto{
	...
	if len(CVC) != 101 {
		return []battery.BatteryDto{}, errors.New("not all records were read correctly")
	}
