...
type Battery struct {
	...
	CRating                json.RawMessage `json:"cRating" gorm:"type:jsonb;not null"`                // С - рейтинг аккумулятора
	InternalResistance     float64         `json:"internalResistance" gorm:"type:float;not null"`     // Внутреннее сопротивление аккумулятора, (Ом)
	CellChemicalProperties json.RawMessage `json:"cellChemicalProperties" gorm:"type:jsonb;not null"` // Химические свойства аккумулятора
